...
- [`pointblank`](https://rstudio.github.io/pointblank/) package for data validation
...
* Note, do not commit .Rhistory files because that is a log of the console, which could send secret info.

## Running Code Repeatedly

- Eliminate all warnings and messages
  
  - Warnings remove. could use `options(warn = 2)` to change all warnings for errors.
  
  - Deprication could change this to an error with `options(lifecycle_verbosity = "error")`
  
  - Messages help clean up the logs for error isolation

- Check and reduce the number of dependencies, causing potential for errors in the future
...
Use the `library(conflicts)` function, and it will throw and error and force you to deal with the conflicts


### `renv`: Install a package per project

> Only use this unless you have to, keeps the code in the original state. 
...
check's column types, missingness, matching variables, values are in range

RMD data validation report
...
- Use parquet instead of CSVs

- duckplyr has dplyr syntax but only pulls data when you need it. Good for large data sets.

"Worst possible case: code only available on one person's computer, being emailed back and forth with others"

- Github tags can be useful
...
Create a quarto book for your style. If we do not want to use it, and style.tidyverse's
...
- GOal is not perfection, but better over time

- Give yourself a couple of days

- Pull request on your own repo

- Use the DEAR (man) framework to ask for feedback. It helps with helping others focus on it.

- Use the GIVE method for feedback (Gentle, Interested, Validate, Easy Manner)
  - Be sure to mention what they did really well. (I never thought of doing it that way.)
  
### Shared Ownership
  
> Strive for the culture of the ability to merge and ask  
